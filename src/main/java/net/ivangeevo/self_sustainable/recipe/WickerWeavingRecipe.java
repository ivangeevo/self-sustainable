...
public class WickerWeavingRecipe implements CraftingRecipe {
    ...
    final int damage; // New field to hold the damage value

    public WickerWeavingRecipe(Identifier id, String group, CraftingRecipeCategory category, int width, int height, DefaultedList<Ingredient> input, ItemStack output, int damage, boolean showNotification) {
        this.id = id;
        this.group = group;
    ...
    public WickerWeavingRecipe(Identifier id, String group, CraftingRecipeCategory category, int width, int height, DefaultedList<Ingredient> input, ItemStack output, boolean showNotification) {
        this(id, group, category, width, height, input, output, 0, showNotification); // Default damage set to 0
    }
    ...
    public ItemStack craft(RecipeInputInventory recipeInputInventory, DynamicRegistryManager dynamicRegistryManager) {
        ItemStack resultStack = this.output.copy(); // Start with a copy of the output stack
        if (resultStack.isDamageable()) {
            int currentDamage = resultStack.getDamage();
            int maxDamage = resultStack.getMaxDamage();
            int newDamage = Math.min(maxDamage, currentDamage + this.damage); // Calculate new damage
            resultStack.setDamage(newDamage); // Apply the new damage to the stack
        }
        return resultStack;
    }
    ...
    static String[] removePadding(String ... pattern) {
        ...
        for (int m = 0; m < pattern.length; ++m) {
            ...
            i = Math.min(i, WickerWeavingRecipe.findFirstSymbol(string));
            int n = WickerWeavingRecipe.findLastSymbol(string);
            j = Math.max(j, n);
            if (n < 0) {
                if (k == m) {
                    ++k;
    ...
    static String[] getPattern(JsonArray json) {
        ...
        if (strings.length == 0) {
            throw new JsonSyntaxException("Invalid pattern: empty pattern");
        }
    ...
    public static ItemStack outputFromJson(JsonObject json) {
        Item item = WickerWeavingRecipe.getItem(json);
        if (json.has("data")) {
        ...
        if (json.has("damage")) {
            int damage = JsonHelper.getInt(json, "damage");
    ...
    public static class Serializer implements RecipeSerializer<WickerWeavingRecipe> {
        ...
        private final RecipeFactory<WickerWeavingRecipe> recipeFactory;
        public static final WickerWeavingRecipe.Serializer INSTANCE = new WickerWeavingRecipe.Serializer(
                (id, group, category, input, output, width, height, damage, showNotification) ->
                        new WickerWeavingRecipe(id, group, category, width, height, input, output, damage, showNotification)
        );
        public static final String ID = "wicker_weaving";

        public Serializer(RecipeFactory<WickerWeavingRecipe> recipeFactory) {
        ...
        public WickerWeavingRecipe read(Identifier identifier, JsonObject jsonObject) {
            ...
            Map<String, Ingredient> map = WickerWeavingRecipe.readSymbols(JsonHelper.getObject(jsonObject, "key"));
            String[] strings = WickerWeavingRecipe.removePadding(WickerWeavingRecipe.getPattern(JsonHelper.getArray(jsonObject, "pattern")));
            int i = strings[0].length();
            int j = strings.length;
            DefaultedList<Ingredient> defaultedList = WickerWeavingRecipe.createPatternMatrix(strings, map, i, j);
            ItemStack itemStack = WickerWeavingRecipe.outputFromJson(JsonHelper.getObject(jsonObject, "result"));
            int damage = JsonHelper.getInt(jsonObject, "damage", 0); // Read damage from JSON or default to 0
            boolean bl = JsonHelper.getBoolean(jsonObject, "show_notification", true);
            return this.recipeFactory.create(identifier, string, craftingRecipeCategory, defaultedList, itemStack, i, j, damage, bl);
        }
        ...
        public WickerWeavingRecipe read(Identifier identifier, PacketByteBuf packetByteBuf) {
            ...
            int damage = packetByteBuf.readInt(); // Read damage from packet
            boolean bl = packetByteBuf.readBoolean();
            return new WickerWeavingRecipe(identifier, string, craftingRecipeCategory, i, j, defaultedList, itemStack, damage, bl);
        }

        @Override
        public void write(PacketByteBuf packetByteBuf, WickerWeavingRecipe shapedRecipe) {
            ...
            packetByteBuf.writeInt(shapedRecipe.damage); // Write damage to packet
            packetByteBuf.writeBoolean(shapedRecipe.showNotification);
        }

        public interface RecipeFactory<T extends CraftingRecipe> {
            T create(Identifier id, String group, CraftingRecipeCategory category, DefaultedList<Ingredient> input, ItemStack output, int width, int height, int damage, boolean showNotification);
        }
    }
}
